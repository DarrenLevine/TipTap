'''
Python DirectServo (pyds.py) interface
...
# the following would be better organized as class static member
# variables/methods, but it's more performant to avoid dot lookups
# in this manner


def Decimal2HexStr(x):
    return hex(abs(x))[2:].zfill(2).upper()[-2:]


d2hi = {x: int.from_bytes(Decimal2HexStr(x).encode('utf-8'), 'big')
        for x in range(256)}
BroadcastID = 0xFF
BROADCAST = d2hi[BroadcastID]
# passes torque parameter: [-255 to 255]
CMD_TORQUE = int.from_bytes(b"T", 'big') << 32
# passes ID parameter: [0 to 255] 0xFF is reserved for broadcast
CMD_SETID = int.from_bytes(b"I", 'big') << 24
...
CMD_FAILSAFE = int.from_bytes(b"F", 'big') << 24
# no passed parameters (resets flash)
CMD_RESET = int.from_bytes(b"R", 'big') << 8
...
def RatioToByte(x):
    return (MINUS_SYM_BYTE if x < 0 else PLUS_SYM_BYTE,
            int((1 if x > 1 or x < -1 else abs(x))*0xFF))
...
def Torque_BYTES(Dest, TorqueRatio):
    ...
    return ((d2hi.get(Dest, BROADCAST) << 40) | CMD_TORQUE |
            ((tr_prefix | d2hi.get(TorqueRatio, 0xFF)) << 8) |
...
def SetID_BYTES(Dest, NewID):
    ...
    return ((d2hi.get(Dest, BROADCAST) << 32) |
            CMD_SETID | (d2hi[NewID] << 8) |
            NEWLINE_BYTE).to_bytes(6, 'big')


def Calibrate_BYTES(Dest):
    return ((d2hi.get(Dest, BROADCAST) << 16) |
            CMD_CALIBRATE |
            NEWLINE_BYTE).to_bytes(4, 'big')
...
def Failsafe_BYTES(Dest,
                   FailoverTorqueRatio,
                   SearchingTorqueRatio,
                   FailAtLimitOffsetDeg):
    FailAtLimitOffsetDeg = 0 if FailAtLimitOffsetDeg < 0 else int(
        FailAtLimitOffsetDeg/360.0*0xFFFF)
...
class DirectServo():
    """A helper class for controlling the DirectServo, it is not built for
    efficiency, (use C/C++ if you need that) but instead from a desire
    to mess around with the controller easily via a python interface.

    Examples:

        YourServo = DirectServo(ID = 23) # initialize a communication object
                                         # to ID 23
        YourServo.SetID()                # send a broadcast signal to
                                         # set ID = 23
        YourServo.Calibrate()            # calibrate servo with ID = 23
        Angle = YourServo.GetAngle()     # get the angle in degrees
        Angle = YourServo.Torque(0.25)   # set torque to 25% and return the
                                         # angle
        Angle = YourServo.SetZeroAngle() # set the angle tracked by the motor
                                         # driver to zero at its current
                                         # position
        Angle = YourServo.SetFailsafe()  # turns on a failsafe feature, and
                                         # passes optional settings:
                                         # 'failsafe':-1.0 to 1.0,
                                         # 'search':0.0 to 1.0,
                                         # 'offset': value in degrees from
                                         # searched limit to set failsafe
                                         # at [0 to 360]
        Angle = YourServo.Reset()        # resets the board and clears the
                                         # flash memory to default settings
    """
    ...
    def __init__(self,
        ...
        if Port is None:
            raise RuntimeError(
                "Must specify a serial port when creating a" +
                " DirectServo object. Example: DirectServo(Port = 'COM1')"
            )
        self.ID = ID
        ...
        if not self.RS485.isOpen():
            raise RuntimeError(
                "{}.RS485({}) connection not open".format(self, self.ID))

    def _Read(self):
        Response = {"ID": BroadcastID, "Angle": None,
                    "Accel": None, "Code": "No Response"}
        try:
        ...
        try:
            ...
            Response["ID"] = int.from_bytes(
                bytes.fromhex(rxline[:2]), byteorder='big')
            Accel = int.from_bytes(bytes.fromhex(rxline[2:4]), 'big')
            Info = rxline[4:]
            if Info != 'error':
                Response["Angle"] = 360.0*(int.from_bytes(bytes.fromhex(
                    Info), byteorder='big', signed=True) / float(0xFFFF))
            ...
            print('Received unrecognized response:' +
                  ' "{}" due to: {}'.format(rxline, e))
            self.RS485.flushInput()
            self.RS485.flushOutput()
        return Response
    ...
    def SetFailsafe(self,
                    FailoverTorqueRatio=0,
                    SearchingTorqueRatio=0.4,
                    FailAtLimitOffsetDeg=10,
                    Timeout=15):
        self.RS485.write(Failsafe_BYTES(
            self.ID, FailoverTorqueRatio,
            SearchingTorqueRatio,
            FailAtLimitOffsetDeg))
        return self._WaitForRead(Timeout)
...
# making sure all connected devices are left at zero torque
# if the user's program fails
...
def KillAndNotify():
    ...
    if UserKillFunction is not None:
        try:
            UserKillFunction()
